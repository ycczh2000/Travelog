...
const WaterfallLayout = () => {
  ...
  const momo = 'https://img1.baidu.com/it/u=1389873612,485301600&fm=253&app=120&size=w931&n=0&f=JPEG&fmt=auto?sec=1712595600&t=76261ab2a1585815f46c7b306c6f66e3'
  useEffect(() => {
        ...
        {data.map((item, index) => (
          <div key={index} className="waterfall-item">
            <InfCard
              id={item.id}
              city={item.Location}
              imageUrl={item.image ? 'http://localhost:8000/images/' + item.image : { momo }}
              title={item.title}
              username={item.username}
              avatar={item.avatar}
              likesCount={item.likesCount}
              avatarUrl={`http://localhost:8000/getAvatar/${item.username}`}
            />
          </div>
        ))}
      </div>
